...
static struct srv6_steer_data *srv6_steer_get_by_dest4(uint16_t vrf_id, const struct ip4_net *dest4) {
	struct nexthop *nh;
	void *data;
...
static struct api_out srv6_steer_add(const void *request, void ** /*response*/) {
	...
	ret = rte_hash_add_key_data(srv6_steer_hash, nh, sd);
	if (ret < 0) {
...
const char *behavior_to_str(enum gr_srv6_behavior b) {
	return behavior_str[b];
}

enum gr_srv6_behavior str_to_behavior(const char *str) {
	int i;
...
static struct api_out srv6_localsid_list(const void *request, void **response) {
	...
	while (rte_hash_iterate(srv6_localsid_hash, &key_ptr, &data_ptr, &iter) >= 0) {
		...
		if (req->vrf_id == UINT16_MAX || req->vrf_id == key->vrf_id) {
			...
			snprintf(ldata.behavior, sizeof (ldata.behavior), "%s",
				 behavior_to_str(data->behavior));
